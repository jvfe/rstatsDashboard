...
number_of_unique_tweets <- rstats_tweets %>% 
  pull(text) %>% 
  unique() %>% 
  length()

number_of_unique_tweets_today <- tweets_today %>%
  pull(text) %>% 
  unique() %>% 
  length()

number_of_tweeters_today <- tweets_today %>% 
  pull(user_id) %>% 
  unique() %>% 
  length()
...
```{r}
valueBox(number_of_tweeters_today, icon = "fa-user", color = "palevioletred")
```

### #rstats Likes

```{r}
valueBox(number_of_likes, icon = "fa-heart", color = "peachpuff")
```
...
### 💗 Most Liked Tweet Today {.tweet-title}
...
shiny::HTML(get_tweet_embed(most_recent$screen_name, most_recent$status_id))
```
